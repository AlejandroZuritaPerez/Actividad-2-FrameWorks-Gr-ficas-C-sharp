        ...
        public void Grafica1 ()
        {
            while (true)
            {
                ...
                for (int i = 0; i < 100; i++)
                {
                    ...
                    int numeroAL = numeroAzar.Next(1, 45);

                    if (chart1.InvokeRequired)
                    {
                        chart1.Invoke(new MethodInvoker(delegate
                        {
                            chart1.Series["Grados"].Points.AddY(numeroAL);
                            chart1.Series["Grados"].Points[i].AxisLabel = i.ToString();
                            chart1.Series["Grados"].YValueType = ChartValueType.String;
                            chart1.Series["Grados"].Points[i].Label = numeroAL.ToString();

                        }));
                    }
                    else
                    {
                        chart1.Series["Grados"].Points.AddXY(numeroAL);
                        chart1.Series["Grados"].Points[i].AxisLabel = i.ToString();
                        chart1.Series["Grados"].YValueType = ChartValueType.String;
                        chart1.Series["Grados"].Points[i].Label = numeroAL.ToString();
                    }
